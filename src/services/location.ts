...
const publicTruckLocationsCol = () =>
    collection(db, "publicTruckLocations");
...
// already added earlier for BrowseTrucksPage reads
export async function listPublicTruckLocations(): Promise<Location[]> {
    const q = query(publicTruckLocationsCol());
    const snap = await getDocs(q);
    return snap.docs.map((d) => d.data() as Location);
}

// create /publicTruckLocations/{id} for browsing
export async function createPublicTruckLocation(params: {
    accountId: string;
    id?: string;
    name: string;
    description?: string;
    ...
    const col = publicTruckLocationsCol();
    const ref = id ? doc(col, id) : doc(col);
